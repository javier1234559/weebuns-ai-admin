...
interface ListeningShowAllViewProps {
...
export default function ListeningShowAllView({
  data,
  isLoading,
  isError,
  error,
  onUpdateQueryParams,
}: ListeningShowAllViewProps) {
  const navigate = useNavigate();
  const [viewMode, setViewMode] = useState<"grid" | "list">("grid");

  const handleViewLesson = (lesson: Lesson) => {
    const routeName = replaceRouteName(
      `${globalConfig.APP_USER_URL}${RouteNames.ListeningDetail}` as RouteNames,
      { id: lesson.id },
    );
  ...
  const handleEditLesson = (lesson: Lesson) => {
    const routeName = replaceRouteName(RouteNames.TeacherListeningUpdate, {
      id: lesson.id,
    });
    navigate(routeName);
  };
      ...
      <div className="w-full mt-4">
        <div className="flex items-center justify-between mb-4">
          <p className="text-sm text-muted-foreground">
            {data.pagination?.totalItems} bài nghe
          </p>
          <Tabs
            value={viewMode}
