...
interface WritingShowAllViewProps {
...
export default function WritingShowAllView({
  data,
  isLoading,
  isError,
  error,
  onUpdateQueryParams,
}: WritingShowAllViewProps) {
  const navigate = useNavigate();
  const [viewMode, setViewMode] = useState<"grid" | "list">("grid");

  const handleViewLesson = (lesson: Lesson) => {
    const routeName = replaceRouteName(
      `${globalConfig.APP_USER_URL}${RouteNames.WritingDetail}` as RouteNames,
      { id: lesson.id },
    );
  ...
  const handleEditLesson = (lesson: Lesson) => {
    const routeName = replaceRouteName(RouteNames.TeacherWritingUpdate, {
      id: lesson.id,
    });
    navigate(routeName);
  };
