...
interface SpeakingShowAllViewProps {
...
export default function SpeakingShowAllView({
  data,
  isLoading,
  isError,
  error,
  onUpdateQueryParams,
}: SpeakingShowAllViewProps) {
  const navigate = useNavigate();
  const [viewMode, setViewMode] = useState<"grid" | "list">("grid");

  const handleViewLesson = (lesson: Lesson) => {
    const routeName = replaceRouteName(
      `${globalConfig.APP_USER_URL}${RouteNames.SpeakingDetail}` as RouteNames,
      { id: lesson.id },
    );
  ...
  const handleEditLesson = (lesson: Lesson) => {
    const routeName = replaceRouteName(RouteNames.TeacherSpeakingUpdate, {
      id: lesson.id,
    });
    navigate(routeName);
  };
